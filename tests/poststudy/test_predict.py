...
class TestLoadPartitionGuards:
    """Test duplicate row ID guards in TaskOutersplitLoader.load_partition."""

    def test_duplicate_row_ids_in_json(self, study_path):
        """Verify ValueError on duplicate row IDs in split_row_ids.json."""
        import json as _json  # noqa: PLC0415

        from octopus.poststudy.study_io import TaskOutersplitLoader  # noqa: PLC0415

        loader = TaskOutersplitLoader(study_path, outersplit_id=0, task_id=0)
        split_ids_path = loader.fold_dir / "split_row_ids.json"
        with split_ids_path.open() as f:
            split_info = _json.load(f)

        # Inject a duplicate into traindev_row_ids
        ...
        try:
            with pytest.raises(ValueError, match="duplicate row IDs"):
                loader.load_partition("traindev")
        finally:
            # Restore original
    ...
    def test_duplicate_row_ids_in_parquet(self, study_path):
        ...
        loader = TaskOutersplitLoader(study_path, outersplit_id=0, task_id=0)
        prepared_path = loader.study_path / "data_prepared.parquet"
        original_data = parquet_load(prepared_path)

        # Create data with duplicate ID values
    ...
    def test_valid_row_ids(self, study_path):
        ...
        loader = TaskOutersplitLoader(study_path, outersplit_id=0, task_id=0)
        result = loader.load_partition("traindev")
    ...
    def test_single_target_returns_target_key(self, tpt):
        ...
        try:
            tpt._study_info = StudyInfo(
                ...
                n_folds_outer=tpt._study_info.n_folds_outer,
...
class TestBuildPoolData:
    ...
    def test_keyerror_fallback(self, tp, study_path, monkeypatch):
        ...
        monkeypatch.setattr(
            TaskOutersplitLoader, "load_partition", lambda *a, **kw: (_ for _ in ()).throw(KeyError("missing row"))
        )
    ...
    def test_filenotfounderror_fallback(self, tp, study_path, monkeypatch):
        ...
        monkeypatch.setattr(
            TaskOutersplitLoader, "load_partition", lambda *a, **kw: (_ for _ in ()).throw(FileNotFoundError())
        )
        data = parquet_load(f"{study_path}/data_prepared.parquet")
        pool = tp._build_pool_data(data)
        for split_id in tp.study_info.outersplits:
...
class TestOctoPredictorPredict:
    ...
    def test_predict_array(self, tp, study_path):
        """Verify OctoPredictor predict returns array matching input length."""
        data = parquet_load(f"{study_path}/data_prepared.parquet")
    ...
    def test_predict_df(self, tp, study_path):
        """Verify OctoPredictor predict with df=True includes ensemble predictions."""
        data = parquet_load(f"{study_path}/data_prepared.parquet")
    ...
    def test_mock_regression_guard(self, tp):
        ...
        try:
            tp._study_info = StudyInfo(
                ...
                n_folds_outer=tp._study_info.n_folds_outer,
