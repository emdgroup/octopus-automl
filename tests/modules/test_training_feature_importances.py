...
# Note: "calculate_fi_shap" (kernel SHAP) is excluded from FI_METHODS above
# because it is too slow/memory-heavy for CI.  It is kept here so that if
# it is ever re-enabled in FI_METHODS, the subprocess guard activates
# automatically.
...
def _get_available_models_by_type() -> dict[MLType, list[ModelName]]:
    ...
    """
    all_models = [ModelName(k) for k in Models._config_factories]
    models_by_type: dict[MLType, list[ModelName]] = {ml_type: [] for ml_type in MLType}
...
def _create_test_data():
    ...
    n_samples = int(TEST_CONFIG["n_samples"])
...
def _create_training_instance(
    ...
    target_assignments: dict[str, str] = dict(config["target_assignments"])  # type: ignore[arg-type]
    target_metric = str(config["target_metric"])
...
def _run_fi_method(training: Training, method_name: str) -> list[str]:
    ...
    Uses ``Training.calculate_fi()`` dispatcher for standard methods.
    ``calculate_fi_featuresused_shap`` uses the private method directly
    because it is a specialised "auto" SHAP variant not exposed via the
    dispatcher.
    """
    from octopus.types import FIComputeMethod  # noqa: PLC0415
    ...
    if method_name == "calculate_fi_internal":
        ...
        training.calculate_fi(FIComputeMethod.PERMUTATION, partition="dev", n_repeats=1)
        return ["permutation_dev"]
    elif method_name == "calculate_fi_lofo":
        training.calculate_fi(FIComputeMethod.LOFO)
        return ["lofo_dev", "lofo_test"]
    elif method_name == "calculate_fi_featuresused_shap":
        training._calculate_fi_featuresused_shap(partition="dev")
        return ["shap_dev"]
    elif method_name == "calculate_fi_shap":
        training.calculate_fi(FIComputeMethod.SHAP, partition="dev", shap_type="permutation")
        return ["shap_dev"]
    else:
...
def _run_in_subprocess(ml_type: MLType, model_name: str, fi_method: str) -> None:
    ...
    """
    # Resolve the repo root so the subprocess can import ``octopus``
    # even when it is not installed as a site-package (editable install).
    # Setting PYTHONPATH to the repo root mirrors what pytest does when
    # it runs from the project directory.
