...
class OuterSplitLoader:
    ...
    def result_dir(self) -> UPath:
        """Get the result type directory path (e.g. task0/best/)."""
        return self.task_dir / self.result_type

    @property
    def module_dir(self) -> UPath:
        """Get the module directory path (contains model and predictor state)."""
        module_dir = self.result_dir / "model"
    ...
    def load_selected_features(self) -> list[str]:
        """Load selected features for this task and result type.
        ...
        """
        sf_path = self.result_dir / "selected_features.json"
    ...
    def load_scores(self) -> pd.DataFrame:
        """Load scores DataFrame for this task and result type.
        ...
        """
        scores_path = self.result_dir / "scores.parquet"
        if scores_path.exists():
            return pd.read_parquet(scores_path, engine="pyarrow")
        return pd.DataFrame()

    def load_predictions(self) -> pd.DataFrame:
        """Load predictions for this task and result type.
        ...
        """
        predictions_path = self.result_dir / "predictions.parquet"
        if not predictions_path.exists():
            raise FileNotFoundError(f"Predictions not found: {predictions_path}")
        df = pd.read_parquet(predictions_path, engine="pyarrow")
        # Filter by module for safety (result_type is implicit from directory)
        if "module" in df.columns and self.module:
            filtered = df[df["module"] == self.module]
            return filtered if not filtered.empty else df
        return df

    def load_feature_importance(self) -> pd.DataFrame:
        """Load feature importance for this task and result type.
        ...
        """
        fi_path = self.result_dir / "feature_importances.parquet"
