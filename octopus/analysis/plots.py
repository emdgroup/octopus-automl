...
def feature_count_plot(feature_df: pd.DataFrame) -> go.Figure:
    ...
    if has_tasks:
        ...
        for s_idx, split_id in enumerate(sorted(df.index.unique())):
            ...
            values = [
                split_df.loc[split_df["task"] == t, "n_features"].iloc[0]
                if t in split_df["task"].values else None
                for t in task_ids
            ]
...
def performance_plot(perf_df: pd.DataFrame, metric: str | None = None) -> go.Figure:
    ...
    metric_cols = [
        ...
        if c != "task" and df[c].dtype in ("float64", "float32", "int64")
    ]
    ...
    if has_tasks:
        ...
        for m_idx, col in enumerate(metric_cols):
            for t_idx, tid in enumerate(task_ids):
                ...
                values = [task_df.loc[s, col] if s in task_df.index else None for s in splits]
...
def aucroc_plot(
    ...
    Args:
        predictor: ``TaskPredictorTest`` instance for a binary
            classification task.
