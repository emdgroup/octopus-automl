...
- [Multiclass Wine](../examples/wf_multiclass_wine.md): Shows how Octopus handles three or more classes using the Wine dataset.
- [ROC Feature Filtering](../examples/wf_roc_octo.md): Demonstrates removing correlated features with ROC before training a model.
- [Multi-Step Classification](../examples/wf_multi_step_classification.md): Chains Octo, MRMR, and Octo into a three-step pipeline that progressively narrows the feature set.
- [Octo & AutoGluon](../examples/wf_octo_autogluon.md): Runs Octo and AutoGluon independently on the same data to compare approaches.
